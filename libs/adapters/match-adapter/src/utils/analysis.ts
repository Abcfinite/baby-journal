...
export default class Analysis {
  ...
  async knn(player1: Player, player2: Player, players: {}) {
    ...
    const distances = await Promise.all(
      finishedHtmlfileList.map( async file => {
        ...
        return {id: file, distance: distance, correctRanking: correctRanking}
      })
    )

    return  distances.filter(dis => dis.correctRanking === true).sort((a, b) => {
